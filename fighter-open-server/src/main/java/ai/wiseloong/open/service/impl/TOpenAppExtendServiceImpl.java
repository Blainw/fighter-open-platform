package ai.wiseloong.open.service.impl;

import ai.wiseloong.open.pojo.TOpenAppExtend;
import ai.wiseloong.open.pojo.vo.TOpenAppExtendVo;
import ai.wiseloong.open.pojo.dto.TOpenAppExtendDto;
import ai.wiseloong.open.mapper.TOpenAppExtendMapper;
import ai.wiseloong.open.service.ITOpenAppExtendService;
import ai.wiseloong.utils.ExcelUploadUtils;
...
import ai.wiseloong.extend.utils.*;
...
public class TOpenAppExtendServiceImpl implements ITOpenAppExtendService {
    ...
    public void dataExport(HttpServletResponse res, QueryCondition<TOpenAppExtendDto> queryCondition) {
        try {
            ...
            ExcelOperation.commExport(res,entity,style,title, xxList,Arrays.asList(colNamesStrArr), Arrays.asList(colCodesStrArr));
        }catch (Exception e){
            e.printStackTrace();
    ...
    public ResultInfo excelImport(MultipartFile file,HttpServletRequest request, HttpServletResponse res) {
        try{
            ...
            if(num>0){
                ...
                UploadFile uploadFile = iuploadFileService.uploadByBytes(excellReader.getByteArray(),ExcelUploadUtils.getUploadFile("开发平台-应用扩展_错误Excel文件.xlsx"),loginInfo);
